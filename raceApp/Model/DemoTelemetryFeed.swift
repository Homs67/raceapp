    ...
    func start() {
        ...
        task = Task.detached { [bus, trackDrive] in
            ...
            while !Task.isCancelled {
                ...
                bus.publish(.gpsVerticalAccuracy, 8 + 2 * sin(t / 11).magnitude, at: now)
                bus.publish(.gpsSpeedAccuracy, 0.4 + 0.2 * sin(t / 7).magnitude, at: now)
                bus.publish(.gpsWallTime, Date().timeIntervalSince1970, at: now)
