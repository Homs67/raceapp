    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text(title ?? context.kind.title)
                ...
                .padding(.leading, context.corner.titleLeading)
                .padding(.trailing, context.corner.titleTrailing)
            Spacer(minLength: 0)
            WidgetView(context: context)
                .frame(maxWidth: .infinity, alignment: .bottomLeading)
                .padding(.leading, context.corner.bodyLeading)
                .padding(.trailing, context.corner.bodyTrailing)
        }
